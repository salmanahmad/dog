...
TODO - Javascript API to make it easier to send listen for root scope
...
(function(scope) {
  ...
  dog.sendListen = function(action, data, context) {
    var request = $.ajax({
      type: "POST",
      url: action,
  ...
  dog.listenUrl = function(listen) {
    return config.baseUrl + "/track/" + listen.track_id + "/" + listen.name;
  }
  ...
  $(".dog-form").live("submit", function() {
    ...
    dog.sendListen($(this).attr("action"), $(this).serializeObject(), context);
    return false;
  })
  ...
  $(".dog-link").live("click", function() {
    ...
    dog.sendListen($(this).attr("href"), data, context);
    return false;
  });
  
  Handlebars.registerHelper('listenLink', function(listen, options) {
    var url = dog.listenUrl(listen);

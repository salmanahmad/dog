...
@Symbol("database.find_value:in:")
public class FindValue extends Function {
	...
	public Signal resume(StackFrame frame) {
		...
		if(arg1 instanceof StructureValue && arg2 instanceof dog.packages.dog.Collection) {
			...
			try {
				DBObject object = dbCollection.findOne(new BasicDBObject("_id", value.getId()));
				returnValue = Value.createFromMongo(object, frame.getRuntime().getResolver());

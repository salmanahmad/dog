...
@Symbol("string.ends_with:suffix:")
public class EndsWith extends Function {
	...
	public Signal resume(StackFrame frame) {
		...
		Value suffix = frame.variables[1];
		Value returnValue;

		if(value instanceof StringValue && suffix instanceof StringValue) {
			StringValue string = (StringValue)value;
			StringValue sstring = (StringValue)suffix;
			if (StringUtils.endsWith(string.value, sstring.value)){

...
public class StartsWith extends Function {
	...
	public Signal resume(StackFrame frame) {
		...
		Value prefix = frame.variables[1];
		Value returnValue;

		if(value instanceof StringValue && prefix instanceof StringValue) {
			StringValue string = (StringValue)value;
			StringValue sstring = (StringValue)prefix;
			if (StringUtils.startsWith(string.value, sstring.value)){

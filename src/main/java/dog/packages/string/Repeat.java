...
public class Repeat extends Function {
	...
	public Signal resume(StackFrame frame) {
		...
		Value times = frame.variables[1];
		Value returnValue;

		if(value instanceof StringValue && times instanceof NumberValue) {
			StringValue string = (StringValue)value;
			NumberValue count = (NumberValue)times;
			returnValue = new StringValue(StringUtils.repeat(string.value, (int)count.value));
		} else {
